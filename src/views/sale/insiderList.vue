<template xmlns:el-col="http://www.w3.org/1999/html">
    <section>
        ...
        <el-dialog title="验证销售是否开通ThankFund账户" :visible.sync="dialogFormVisible" width="400px">
            <el-form ref="addSaleInfo" :model="addSaleInfo" :rules="rules" label-width="80px" size="small">
                <el-form-item label="销售姓名" prop="sales_name">
                    <el-input v-model="addSaleInfo.sales_name"> </el-input>
                </el-form-item>
        ...
        <div class="wage-operation-info">
            <el-dialog
                title="新增销售"
                :visible.sync="addinsideDialogVisible"
            >
                <addInsider :params="addinside" @closeAddDiaglog="closeAddDiaglog"> </addInsider>
            </el-dialog>
        </div>
    </section>
...
<script>
    import {getInsiderList, isVerifyInsider} from '../../api/sale_api';
    import dict from '../../common/js/dict';
    import {all_identity_no,userName} from '../../common/js/checkFields';
    import addInsider from './AddInsider.vue';

    export default {

        data() {
            return {
                addinsideDialogVisible:false,
                listLoading: false,
                pager: {
        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        let params = {
                            ...
                            name: this.addSaleInfo.sales_name
                        };

                        isVerifyInsider(params).then((res) => {
                            let result = JSON.parse(res.data.datas);
                            if (res.data.status === 200) {
                                this.addinside = result;
                                this.dialogFormVisible = false;
                                this.addinsideDialogVisible = true;
                            } else {
                                if (res.data.status === 401) {
                                    this.$router.push('/login');
                                }
            ...
            closeAddDiaglog:function(data){
                this.addinsideDialogVisible = data;
                this.getItems();
            }
