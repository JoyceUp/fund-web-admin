<template>
    <section>
        ...
        <el-dialog  :title="dialogTitle" :visible.sync="dialogVisible" :before-close="handleClose" width="1000px">
            <el-form :model="addForm" :rules="rules" ref="addForm" label-width="80px" class="demo-ruleForm" :class="dialogDisable?'from-text':''">
                ...
                <el-row :gutter="30"  v-if="addForm.state==3 || addForm.state==4">
                    <el-col :span="8">
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click=" handleClose() " v-if="!dialogDisable">取 消</el-button>
                <el-button type="primary" @click="add()" v-if="!dialogDisable">确 定</el-button>
            </div>
        </el-dialog>
        ...
        >
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                    <el-button type="primary"  @click="showDetails(scope.$index,scope.row) ">查看</el-button>
                 </template>
            </el-table-column>
        </el-table>
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    amount: [
                        { required: true, validator: checkFields.amount, trigger: 'blur' }
                    ],
                    contract_no: [
...
//                  contentType:'application/json;charset=utf-8, charset=UTF-8',
                    ...
                    success : function(res) {
                        if (res.status == 200) {
                            ...
                            if(JSON.parse(res.datas).items.length==0){
                                _this.addForm.customer_id =""
                                _this.addForm.customer_name =""
                                _this.addForm.mobile = ""
                                _this.$message({
                                    type: 'error',
                                    message: "未查询到此用户"
                                });
            ...
            beforeAvatarUpload (file, fileList) {
             //   this.file_list = fileList
              //  console.log(fileList)
                ...
                const extension = fileName[fileName.length-1].toLowerCase()  === 'jpg'
                const extension2 =  fileName[fileName.length-1].toLowerCase() === 'png'
                const extension3 =  fileName[fileName.length-1].toLowerCase() === 'jpeg'
                const extension4 =  fileName[fileName.length-1].toLowerCase() === 'bmp'
                const isLt2M = file.size / 1024 / 1024 < 2

                if (!extension && !extension2 && !extension3 && !extension4) {
                    this.$message({
                        message: '上传图片只能是.jpg、.png、.jpeg、.bmp格式!',
                        type: 'warning'
                    });
                }
                ...
                if ((extension || extension2 || extension3 || extension4) && isLt2M == true) {
